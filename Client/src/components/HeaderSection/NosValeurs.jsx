...
const NosValeurs = () => {
  ...
  const fetchValeurs = async () => {
    try {
      ...
      console.error(err);
      setError("Impossible de charger les valeurs.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-gray-400">Chargement des valeurs...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Nos Valeurs</h1>

      <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
        {valeurs.length === 0 ? (
          <p className="text-gray-500 col-span-full text-center">Aucune valeur disponible.</p>
        ) : (
          valeurs.map((valeur) => (
            <div
              key={valeur.id}
            ...
            >
              ...
              <div className="p-4">
                <h2 className="text-xl font-bold text-gray-800 mb-1">{valeur.title_fr}</h2>
                {valeur.title_en && (
                  <p className="text-gray-600 mb-2">{valeur.title_en}</p>
                )}
                <p className="text-gray-500 text-sm">{valeur.content_fr}</p>
                {valeur.content_en && (
                  <p className="text-gray-500 text-sm">{valeur.content_en}</p>
                )}
              </div>
            </div>
          ))
