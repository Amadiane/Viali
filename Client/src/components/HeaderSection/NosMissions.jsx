...
const NosMissions = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0e27] py-16 px-4">
      <h1 className="text-4xl md:text-6xl font-black text-white text-center mb-12">
        {t("missions.title", "NOS MISSIONS")}
      </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {missions.map((mission) => (
          <div
            key={mission.id}
            className="bg-[#0f1729]/90 backdrop-blur-xl rounded-2xl overflow-hidden border-2 border-orange-500/30 shadow-2xl"
          >
            {mission.image_url ? (
              <img
                src={mission.image_url}
                ...
                onError={(e) =>
                  (e.target.src = "https://placehold.co/600x400/1a1a2e/ffffff?text=Nos+Missions")
                }
              />
            ) : (
              <div className="w-full h-48 bg-orange-500/20 flex items-center justify-center">
                <Target className="w-12 h-12 text-orange-400/50" />
              </div>
            )}
              ...
              {mission.created_at && (
                <div className="flex items-center gap-2 text-gray-300 mt-3">
                  ...
                  <span className="text-xs">
                    {new Date(mission.created_at).toLocaleDateString(i18n.language, { day: 'numeric', month: 'long', year: 'numeric' })}
                  </span>
                </div>
              )}
            </div>
