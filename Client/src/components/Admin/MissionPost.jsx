  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        method,
        headers: getAuthHeaders(),          // ← token JWT
  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* MESSAGES */}
        {error          && <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6 flex items-center justify-between"><span className="text-red-700 font-medium">{error}</span><button onClick={() => setError(null)}><X size={18} className="text-red-500"/></button></div>}
        {successMessage && <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6 flex items-center justify-between"><span className="text-green-700 font-medium">{successMessage}</span><button onClick={() => setSuccessMessage(null)}><X size={18} className="text-green-500"/></button></div>}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <textarea name="content_valeur_fr" value={formData.content_valeur_fr} onChange={handleChange} rows="5"
                      ...
                      className="w-full px-4 py-3 border border-yellow-300 rounded-xl focus:border-[#FDB71A] focus:ring-2 focus:ring-[#FDB71A]/20 transition-all bg-white font-medium resize-none"></textarea>
                  </div>
                  <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <textarea name="content_mission_fr" value={formData.content_mission_fr} onChange={handleChange} rows="5"
                      ...
                      className="w-full px-4 py-3 border border-orange-300 rounded-xl focus:border-[#F47920] focus:ring-2 focus:ring-[#F47920]/20 transition-all bg-white font-medium resize-none"></textarea>
                  </div>
                  <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div className="space-y-3">
                  <label className="font-semibold text-gray-700 text-sm flex items-center gap-2">
                    <ImageIcon className="w-4 h-4 text-[#F47920]"/> Image
                  </label>
                    ...
                    <div className="flex justify-center mt-4">
                      <div className="relative bg-white border border-gray-200 rounded-2xl p-4 shadow-lg w-48 h-48">
                        ...
                        {editingId && typeof formData.image === "string" && (
                          <span className="absolute top-2 left-2 bg-blue-500 text-white px-2 py-1 rounded text-xs font-semibold">Image actuelle</span>
                        )}
                      </div>
                    </div>
                  )}
          ...
          <div className="bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden">
            <div className="p-6 md:p-8 border-b border-gray-200">
              <h3 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
                Liste des missions
                <span className="bg-gradient-to-r from-[#FDB71A] to-[#F47920] text-white px-3 py-1 rounded-full font-semibold text-sm">{missions.length}</span>
              </h3>
            </div>
            <div className="p-6 md:p-8">
              {loading ? (
                ...
                <div className="text-center py-12">
                  <div className="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4"><Target className="w-8 h-8 text-gray-400"/></div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 p-4">
                          {mission.image_url && (
                            <div className="relative w-full md:w-48 h-48 flex-shrink-0 overflow-hidden rounded-xl">
                              <img src={mission.image_url} alt={mission.title_fr}
          ...
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {selectedMission.image_url && (
                <img src={selectedMission.image_url} className="w-full h-80 object-cover rounded-xl mb-6" alt=""/>
              )}
                  ...
                  <div>
                    ...
                    {selectedMission.content_valeur_fr && (
                      <div className="bg-yellow-50 p-4 rounded-xl border-l-4 border-yellow-500 mb-3">
                        ...
                        <p className="text-gray-700 whitespace-pre-wrap">{selectedMission.content_valeur_fr}</p>
                      </div>
                    )}
                    {selectedMission.content_valeur_en && (
                      <div className="bg-yellow-50 p-4 rounded-xl border-l-4 border-yellow-400 mb-3">
                        ...
                        <p className="text-gray-700 whitespace-pre-wrap">{selectedMission.content_valeur_en}</p>
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div>
                    ...
                    {selectedMission.content_mission_fr && (
                      <div className="bg-orange-50 p-4 rounded-xl border-l-4 border-orange-500 mb-3">
                        <h4 className="font-semibold text-gray-700 mb-2 text-sm">Français</h4>
                        <p className="text-gray-700 whitespace-pre-wrap">{selectedMission.content_mission_fr}</p>
                      </div>
                    )}
                    {selectedMission.content_mission_en && (
                      <div className="bg-orange-50 p-4 rounded-xl border-l-4 border-orange-400 mb-3">
                        <h4 className="font-semibold text-gray-700 mb-2 text-sm">English</h4>
                        <p className="text-gray-700 whitespace-pre-wrap">{selectedMission.content_mission_en}</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="bg-gray-50 p-6 flex justify-end gap-3 border-t border-gray-200">
              <button onClick={() => { handleEdit(selectedMission); setSelectedMission(null); window.scrollTo({ top:0, behavior:"smooth" }); }}
