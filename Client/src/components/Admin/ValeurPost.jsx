...
//                 {editingId ? "✏️ Modifier la valeur" : "➕ Ajouter une valeur"}
//               </h2>
...
//                 <div>
//                   <label className="block font-semibold text-gray-300 mb-2">🖼️ Image de la valeur *</label>
//                   <input
//                     type="file"
//                     name="image"
//                     accept="image/*"
...
const ValeurPost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMessage(editingId ? "Valeur mise à jour !" : "Valeur ajoutée !");
      resetForm();
      setShowForm(false);
      fetchValeurs();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Voulez-vous supprimer cette valeur ?")) return;
    try {
      const res = await fetch(CONFIG.API_VALEUR_DELETE(id), { method: "DELETE" });
      if (!res.ok) throw new Error("Erreur de suppression");
      setSuccessMessage("Valeur supprimée !");
      fetchValeurs();
    } catch (err) {
      console.error(err);
      setError("Erreur lors de la suppression");
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-white">Gestion des Valeurs</h1>
        <button
          onClick={() => {
            setShowForm(!showForm);
            if (showForm) resetForm();
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-gray-900 p-6 rounded-lg mb-8 space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="block text-gray-300 mb-1">Titre (FR)</label>
              <input
                type="text"
                name="title_fr"
                value={formData.title_fr}
            ...
            <div>
              <label className="block text-gray-300 mb-1">Title (EN)</label>
              <input
                type="text"
                name="title_en"
                value={formData.title_en}
          ...
          <div>
            <label className="block text-gray-300 mb-1">Description (FR)</label>
            <textarea
              name="content_fr"
          ...
          <div>
            <label className="block text-gray-300 mb-1">Description (EN)</label>
            <textarea
              name="content_en"
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
            />
            {preview && <img src={preview} alt="Aperçu" className="mt-2 w-32 h-32 object-cover" />}
          </div>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? <Loader2 className="animate-spin" size={18} /> : editingId ? "Mettre à jour" : "Ajouter"}
          </button>
        </form>
      )}
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {valeurs.map((valeur) => (
          <div key={valeur.id} className="bg-gray-800 rounded-lg overflow-hidden shadow-lg">
            {valeur.image_url && (
              <img src={valeur.image_url} alt={valeur.title_fr} className="w-full h-40 object-cover" />
            )}
            <div className="p-4">
              <h3 className="text-white font-bold">{valeur.title_fr}</h3>
              {valeur.title_en && <p className="text-gray-300">{valeur.title_en}</p>}
              <p className="text-gray-400 text-sm mt-2">{valeur.content_fr}</p>
              {valeur.content_en && <p className="text-gray-400 text-sm">{valeur.content_en}</p>}

              <div className="flex gap-2 mt-4">
                <button
                  onClick={() => handleEdit(valeur)}
                  className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-1 rounded text-sm flex items-center justify-center gap-1"
                >
                  <Edit2 size={14} /> Modifier
                </button>
                <button
                  onClick={() => handleDelete(valeur.id)}
