...
const ThonRecipesPost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingId
        ? CONFIG.API_THON_UPDATE(editingId)
        : CONFIG.API_THON_CREATE;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(CONFIG.API_THON_DELETE(id), { method: "DELETE" });
      fetchItems();
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-[#0a0e27] p-4 md:p-8">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {items.map((item) => (
          <div
            key={item.id}
            className="bg-[#1a1f3a] p-4 rounded-lg relative"
          >
            {item.image_url && (
              <img
                src={item.image_url}
            ...
            <div className="flex gap-2 mt-3">
              <button
                ...
                className="bg-blue-500 px-3 py-1 text-white rounded flex items-center gap-1"
              >
                <Edit2 size={14} /> Modifier
              </button>
              <button
                onClick={() => handleDelete(item.id)}
                className="bg-red-500 px-3 py-1 text-white rounded flex items-center gap-1"
              >
